...
from app.schemas.document import FolderCreate, DocumentRename, DocumentOut
...
@router.post("/folder", response_model=DocumentOut)
def create_folder(
    ...
    return success_response(
        ...
        data=DocumentOut.model_validate(folder).model_dump(mode="json"),
...
def get_document(
    ...
    doc = document_repo.get_owned_document_by_id(db, document_id, user.id)
    if not doc:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
def rename_document(
    ...
    doc = doc_service.rename_document_service(
        db=db,
        doc_id=document_id,
        user_id=user.id,
        new_name=payload.file_name,
    )
